...
function ScenarioFullView() {
  ...
  const { scenario_id, scenario_page } = useParams();
  console.log(`Received params for instance ${scenario_id} and scenario_page ${scenario_page}`);


  if (!scenario_id) { return (<>Scenario not found</>); };
  ...
  const chosenPage = recalledPage.scenario_page || "home";
...
//--------------------------
  ...
  useEffect(() => {
    // set_guide_uid_state(Number(scenario_id));
    // set_guidePage_state(Number(scenario_page) || "home");
    localStorage.setItem('guidePage', JSON.stringify({
      scenario_id: scenario_id,
      scenario_page: scenario_page}
    ));
