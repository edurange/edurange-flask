...
################
# rendering is done by React and individual API calls will already return an error, so
# this should either be cut or redone to just report the error itself (Which seems redundant)
# cutting
...
################
# potentially useful for React - leaving for now
...
# all of the following rendering and view related logic is completely redundant / unnecessary with React, 
# but should be kept until migration is fully complete so legacy routing doesn't get broken.

################
# start redundant rendering logic
################
...
def tempMaker(sId, i):
    ...
    if i == "ins":
        ...
        return status, ownerName, createdAt, ty, sName, guide, questions
    elif i == "stu":
        ...
        return status, ownerName, ty, sName, username, pw, guide, questions
