...
export const Auth_ContextProvider = ({ children }) => {
    ...
    const { jwt_authenticated_state } = useContext(MainFrameContext);

    if ( jwt_authenticated_state === true) { return children }
    else { return <Login />; }
};

    // This is a component that will only Only render its child components if jwt_authenticated_state is true.
