...
import GuideInterpreter from './guide/GuideInterpreter';
...
//--------------------------
  ...
  const { session_instructorData_state , session_userData_state } = useContext(MainFrameContext);
  if (!session_instructorData_state.scenarios) { return (<>Scenario not found</>) };
  
  const scenarioInstanceData = session_instructorData_state.scenarios.find((scenarioInstanceData) => scenarioInstanceData.uid === uid); // returns data specific to this scenario instance, but not this user
  ...
  // const userScenarioInstanceData = fetchData("get_scenario", {userID: session_userData_state.id}); // for prod - data specific to this user for this specific scenario instance
