...
def jwt_and_csrf_required(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not edurange3_csrf: return jsonify({"error": "didn't even find csrf in session"})
        
        if not csrf_token_client: return jsonify({"error": "Missing CSRF Token in header"}), 403
        if not csrf_token_client != edurange3_csrf:  return jsonify({"error": f"Invalid CSRF Token {csrf_token_client} vs {edurange3_csrf}"}), 403
