...
output "instances" {
  value = [{
    name = "elf"
    ip_address_public  = aws_instance.elf.public_ip
    ip_address_private = aws_instance.elf.private_ip
  }]
}
...
resource "aws_key_pair" "key" {
  key_name   = "elf/${var.scenario_id}"
  public_key = tls_private_key.key.public_key_openssh
}


data "template_cloudinit_config" "elf" {
  gzip          = true
  base64_encode = true
  ...
  part {
    ...
    content = templatefile("${path.module}/cloud-init.yml.tpl", {
      ...
      packages = ["gdb", "build-essential", "colordiff", "strace", "nasm"]
      hostname = "elf"
    })
  }
}
...
resource "aws_route_table_association" "elf_subnet_route_table_association"{
...
resource "aws_instance" "elf" {
  ami                    	 = "ami-0ae370263666f25c5"
  instance_type          	 = "t2.nano"
  private_ip 		 	 = "10.0.0.5"
  associate_public_ip_address    = true
  source_dest_check 		 = false
  subnet_id 		 	 = aws_subnet.public.id
  user_data_base64       	 = data.template_cloudinit_config.elf.rendered
  ...
  provisioner "file" {
    ...
    destination = "/home/ubuntu/elf/install"
  }
  ...
  provisioner "file" {
    ...
    destination = "/home/ubuntu/elf/tty_setup"
  }
